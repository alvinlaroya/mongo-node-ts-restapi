    ...
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                ...
                refreshToken = lodash_1.get(req, "headers.x-refresh");
                if (!accessToken)
                    return [2 /*return*/, next()];
                _a = jwt_utils_1.decode(accessToken), decoded = _a.decoded, expired = _a.expired;
                if (decoded) {
                    // @ts-ignore
                    req.user = decoded;
                    return [2 /*return*/, next()];
                ...
                return [4 /*yield*/, session_service_1.reIssueAccessToken({ refreshToken: refreshToken })];
            case 1:
                newAccessToken = _b.sent();
                if (newAccessToken) {
                    // Add the new access token to the response header
                    res.setHeader("x-access-token", newAccessToken);
                    decoded_1 = jwt_utils_1.decode(newAccessToken).decoded;
                    // @ts-ignore
                    req.user = decoded_1;
                }
                return [2 /*return*/, next()];
